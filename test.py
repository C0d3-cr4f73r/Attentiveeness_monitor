...
if __name__ == "__main__":
    ...
    with mp_holistic.Holistic(
        ...
        # Track start time for minute calculation
        ...
        while cap.isOpened():
            # Capture frame and perform detection/prediction
            ret, frame = cap.read()
            ...
            cv2.putText(image, prediction, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            ...
            if elapsed_time >= 60:
                # Calculate average attentiveness for the past minute
                minute_predictions = predictions[-60:]  # Get last 60 predictions

                if minute_predictions:
                    # Count occurrences of "Attentive" and "Not attentive"
                    attentive_count = minute_predictions.count("Attentive")
                    not_attentive_count = minute_predictions.count("Not attentive")

                    total_predictions = len(minute_predictions)

                    # Calculate average as ratio of counts (avoid division by zero)
                    if total_predictions > 0:
                        average_prediction = (attentive_count / total_predictions, not_attentive_count / total_predictions)
                    else:
                        average_prediction = ("No predictions recorded in the past minute", "No predictions recorded in the past minute")
    ...
    # Save data to CSV
                    with open(csv_file_path, "a", newline="") as csvfile:
                        ...
                        writer.writerow([timestamp_str, average_prediction])
            ...
            # Save any remaining predictions after the last minute
            if predictions:
                attentive_count = predictions.count("Attentive")
                not_attentive_count = predictions.count("Not attentive")

                total_predictions = len(predictions)
                minute_predictions = predictions[-60:]

    # Calculate average as ratio of counts (avoid division by zero)
                if total_predictions > 0:
                     attentive_count = sum(1 for prediction in minute_predictions if prediction == "Attentive")
                     not_attentive_count = sum(1 for prediction in minute_predictions if prediction == "Not attentive")
                     if attentive_count> not_attentive_count:
                         prediction_finale="Attentive"
                     else:
    ...
    # Save average prediction and timestamp to CSV
                with open(csv_file_path, "a", newline="") as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow([timestamp_str,prediction_finale])
